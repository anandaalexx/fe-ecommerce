...
import { Search, ShoppingCart, Store, CircleUserRound, Wallet } from "lucide-react";
...
const Navbar = () => {
  ...
  const fetchSaldo = async () => {
    try {
      ...
      console.log("Saldo user:", data);
      setSaldo(data.saldo); // <== Simpan saldo ke state
    } catch (err) {
    ...
    <nav className="fixed w-full z-99 border-b border-gray-300 bg-white">
      ...
      <div className="flex items-center justify-between py-4 px-6">
        ...
        <div className="flex items-center gap-6">
          {[{ icon: <Store size={24} />, label: "Toko Anda" }, { icon: <ShoppingCart size={24} />, label: "Keranjang" }, { icon: <CircleUserRound size={24} />, label: "Akun" }].map((item, i) => (
            <div
              key={i}
      ...
      <div className="flex justify-between items-center py-3 border-t border-gray-300 text-sm">
        ...
        <div className="flex font-medium text-md justify-left gap-6 flex-1 px-6">
          {[...Array(6)].map((_, i) => (
            <span key={i} className="cursor-pointer relative hover:text-[#EDCF5D] transition-all duration-300">
              Menu
            </span>
          ))}
        </div>
        ...
        <div className="px-6 font-semibold flex items-center gap-2 text-lg cursor-pointer">
          <Wallet size={20} />
          <span>{saldo !== null ? `Rp ${saldo}` : "Loading..."}</span>
        </div>
      </div>
    </nav>
  );
